describe("MEPH/math/Hamilton.spec.js", function () {
    ...
    it('a hamilton can be created', function (done) {
        MEPH.requires('MEPH.math.Hamilton').then(function () {
            var hamilton = new Hamilton(new Vector([1, 2, 3, 4]), new Vector([1, 2, 3, 4]));
    ...
    it('hamilton multiplication', function (done) {
        MEPH.requires('MEPH.math.Hamilton').then(function () {
            ...
            expect(vector.equals(new Vector([-28, 4, 6, 8]))).theTruth('the wrong vector was created');
        }).catch(function () {
            expect(new Error('something went wrong while creating a quaternion')).caught();
        }).then(function (x) {
            done();
    ...
    it('hamilton addition', function (done) {
        MEPH.requires('MEPH.math.Hamilton').then(function () {
            ...
            expect(vector.equals(new Vector([2, 4, 6, 8]))).theTruth('the wrong vector was created');
        }).catch(function () {
            expect(new Error('something went wrong while creating a quaternion')).caught();
        }).then(function (x) {
            done();
